...
class GradeTextRepo(GradeMemoryRepo):
    ...
    def __load(self) -> None:
        ...
        # Reached here, the file is not empty, so load from file to memory
        fin.close()
        self._assignments: dict[int, Grade] = {}
        fin = open(self.__file_path, "r")
        lines = fin.readlines()
        for line in lines:
    ...
    def __save(self):
        ...
        for student in self._grades:
            for grade in self._grades[student].values():
                fout.write(f"{grade}\n")
        fout.close()
    ...
    def update_grade(self, student_id: int, assignment_id: int, new_grade: int):
        super().update_grade(student_id, assignment_id, new_grade)
        self.__save()
    ...
    def remove_assignment(self, student_id: int, assignment_id: int):
        ...
        :return: None
        """
        super().remove_assignment(student_id, assignment_id)
        self.__save()