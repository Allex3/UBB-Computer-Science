...
class StudentTextRepo(StudentMemoryRepo): # pragma: no cover
...
class AssignmentTextRepo(AssignmentMemoryRepo): # pragma: no cover
    def __init__(self, file_path: str):
        super().__init__()
        self.__file_path = file_path
        self.__load()
...
class GradeTextRepo(GradeMemoryRepo): # pragma: no cover
    def __init__(self, file_path: str):
        super().__init__()
        self.__file_path = file_path
        self.__load()
    ...
    def __save(self):
        fout = open(self.__file_path, "w", encoding="utf-8")
        for grade in self._grades.values():
            fout.write(f"{grade}\n")
        fout.close()
