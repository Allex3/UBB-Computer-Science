...
global _base_16_char_to_base_16_number

_base_16_char_to_base_16_number:
    ...
    ; because in string format it is in big-endian normal order
        ...
        mov dword[number], 0
        cld  
        ; [ebp] now has the old ebp there, so [ebp+4] is the return address, so start from ebp+8
        mov ECX, [ebp+12] ; for the loop, this is the number of characters in the current number of the string
        mov esi, [ebp+8] ; hexa_string
        mov edi, number
        cld
        jecxz .end_of_func
        .convert_to_number:
    ...
    ret ; remove the 2 params from the stack
